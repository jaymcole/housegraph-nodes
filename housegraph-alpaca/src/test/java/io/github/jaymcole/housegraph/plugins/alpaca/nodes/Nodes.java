 ...
 * {@code housegraph-robinhood}'s and {@code housegraph-database}'s node tests use.
 */
final class Nodes {
    ...
    /** One of a node's flow outputs, by name. */
    static FlowPort flowOut(BaseNode node, String name) {
        return node.getFlowOutputs().stream()
                .filter(port -> port.name.equals(name))
                .findFirst()
                .orElseThrow(() -> new AssertionError(node.getClass().getSimpleName()
                        + " has no flow output named \"" + name + "\" - it has "
                        + flowOutputNames(node)));
    }

    private static NodeVariable<?> find(List<NodeVariable> variables, String name, BaseNode node,
                                        String kind) {
        return variables.stream()
                .filter(variable -> variable.name.equals(name))
                .findFirst()
                .orElseThrow(() -> new AssertionError(node.getClass().getSimpleName() + " has no " + kind
                        + " named \"" + name + "\" - it has "
                        + variables.stream().map(variable -> variable.name).toList()));
    }
}
